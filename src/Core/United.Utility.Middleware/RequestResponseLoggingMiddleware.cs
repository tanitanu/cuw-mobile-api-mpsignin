...
namespace United.Utility.Middleware
{
    public class RequestResponseLoggingMiddleware
    {
        ...
        public async Task InvokeAsync(HttpContext httpContext, CacheLogWriter cacheLog)
        {
            try
            {
                ...
                if (httpContext.Request.Path.HasValue && httpContext.Request.Path.Value.ToLower().Contains("/api"))
                {
                        ...
                        && !httpContext.Request.Path.Value.ToLower().Contains("/api/version"))
                    {
                        ...
                        await LogHttpRequest(httpContext.Request).ConfigureAwait(false);
        ...
        private async Task LogHttpResponse(HttpResponse response)
        {
