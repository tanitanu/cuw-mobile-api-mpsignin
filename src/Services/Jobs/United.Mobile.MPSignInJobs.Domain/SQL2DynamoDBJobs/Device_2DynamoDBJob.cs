...
{
    public class Device_2DynamoDBJob : IHostedService, IDisposable
    {
        private readonly ICacheLog<Device_2DynamoDBJob> _logger;
        ...
        public Device_2DynamoDBJob(
            ICacheLog<Device_2DynamoDBJob> logger
        ...
        public Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            if (startFlag)
            {
                ...
                var triggerJobInSeconds = _configuration.GetValue<int>("TriggerDevice_2DynamoDBJobInSeconds") == 0 ? 3600 :
                                          _configuration.GetValue<int>("TriggerDevice_2DynamoDBJobInSeconds");
                ...
                timer = new Timer(tsk =>
                {
                    try
                    {
                        _logger.LogInformation("Device_2DynamoDBJob Started Iteration : {iterationIndex}", iterationIndex);

                        Interlocked.Increment(ref iterationIndex);
                        _sql2DynamoDBJobBusiness.uatb_Device(result);
                    }
                    catch (Exception ex)
                    {
                        _logger.LogError("Device_2DynamoDBJob uatb_Device failed at Iteration : {iterationIndex} with exception: {message}", iterationIndex, ex.Message);
