...
{
    public class EResBetaTester_2DynamoDBJob : IHostedService, IDisposable
    {
        private readonly ICacheLog<EResBetaTester_2DynamoDBJob> _logger;
        ...
        public EResBetaTester_2DynamoDBJob(
            ICacheLog<EResBetaTester_2DynamoDBJob> logger
        ...
        public Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            if (startFlag)
            {
                var triggerJobInSeconds = _configuration.GetValue<int>("TriggerEResBetaTester_2DynamoDBJobInSeconds") == 0 ? 3600 :
                                 _configuration.GetValue<int>("TriggerEResBetaTester_2DynamoDBJobInSeconds");
                ...
                timer = new Timer(tsk =>
                {
                    try
                    {
                        _logger.LogInformation("EResBetaTester_2DynamoDBJob Started Iteration : {iterationIndex}", iterationIndex);
                        Interlocked.Increment(ref iterationIndex);
                        _sql2DynamoDBJobBusiness.uatb_EResBetaTester(result);
                    }
                    catch (Exception ex)
                    {
                        _logger.LogError("EResBetaTester_2DynamoDBJob failed at Iteration : {iterationIndex} with exception: {message}", iterationIndex, ex.Message);
