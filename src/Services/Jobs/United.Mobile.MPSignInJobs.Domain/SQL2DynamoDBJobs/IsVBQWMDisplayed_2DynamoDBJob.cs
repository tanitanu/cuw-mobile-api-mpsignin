...
{
    public class IsVBQWMDisplayed_2DynamoDBJob : IHostedService, IDisposable
    {
        private readonly ICacheLog<IsVBQWMDisplayed_2DynamoDBJob> _logger;
        ...
        public IsVBQWMDisplayed_2DynamoDBJob(
            ICacheLog<IsVBQWMDisplayed_2DynamoDBJob> logger
        ...
        public Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            if (startFlag)
            {
                var triggerJobInSeconds = _configuration.GetValue<int>("TriggerIsVBQWMDisplayed_2DynamoDBJobInSeconds") == 0 ? 3600 :
                                _configuration.GetValue<int>("TriggerIsVBQWMDisplayed_2DynamoDBJobInSeconds");
                ...
                timer = new Timer(tsk =>
                {
                    try
                    {
                        _logger.LogInformation("IsVBQWMDisplayed_2DynamoDBJob Started Iteration : {iterationIndex}", iterationIndex);

                        Interlocked.Increment(ref iterationIndex);
                        _sql2DynamoDBJobBusiness.uatb_IsVBQWMDisplayed(result);
                    }
                    catch (Exception ex)
                    {
                        _logger.LogError("IsVBQWMDisplayed_2DynamoDBJob failed at Iteration : {iterationIndex} with exception: {message}", iterationIndex, ex.Message);
