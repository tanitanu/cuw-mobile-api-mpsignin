...
{
    public class StageOnPremService : IStageOnPremService
    {
        private readonly ICacheLog<StageOnPremService> _logger;
        private readonly IResilientClient _resilientClient;

        public StageOnPremService(ICacheLog<StageOnPremService> logger, [KeyFilter("OnPremSQLStageClientKey")] IResilientClient resilientClient)
        {
            _logger = logger;
        ...
        public async Task<T> GetCatalogServiceDetails<T>(string applicationId, string deviceId = "DeviceID", string SessionId = "SessionID", bool IsCloudCatalog = true)
        {
            ...
            string path = "/UnitedMobileDataServicesCloudMigration/api/Catalog/GetCatalogItemsV2";
